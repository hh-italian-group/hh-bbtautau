    ...
    void Run()
    {
        for(const SampleEntry& entry:samples)
        {
            ...
            for(Long64_t current_entry = 0; current_entry < std::min(tuple.GetEntries(),args.number_events()); ++current_entry) {
                ...
                if (event.eventEnergyScale!=0 || (event.q_1+event.q_2)!=0 || event.jets_p4.size() < 2) continue;
...
//                vars["transversemass_hl"]=std::sqrt(2*leptons.pt()*event.pfMET_p4.Et()*(1-std::cos(std::abs(ROOT::Math::VectorUtil::DeltaPhi(leptons, event.pfMET_p4)))));
        ...
        factory->BookMethod( dataloader.get(), TMVA::Types::kBDT, "BDT",
                                    "!H:!V:NTrees=100:MinNodeSize=2.5%:MaxDepth=3:BoostType=AdaBoost:"
                                    "UseBaggedBoost:BaggedSampleFraction=0.5:SeparationType=GiniIndex:nCuts=-1" );

        /*factory->BookMethod(dataloader.get(), TMVA::Types::kBDT, "BDT","!H:!V:NTrees=400:"
        "nEventsMin=400:MaxDepth=3:BoostType=AdaBoost:SeparationType=GiniIndex:nCuts=-1:PruneMethod=NoPruning");*/

        factory->BookMethod( dataloader.get(), TMVA::Types::kBDT, "BDTD",
                                    "!H:!V:NTrees=100:MinNodeSize=2.5%:MaxDepth=3:BoostType=AdaBoost:"
                                     "SeparationType=GiniIndex:VarTransform=Decorrelate:nCuts=-1" );

        factory->BookMethod( dataloader.get(), TMVA::Types::kBDT, "BDTF",
                                    "!H:!V:NTrees=100:MinNodeSize=2.5%:UseFisherCuts:MaxDepth=3:"
                                    "BoostType=AdaBoost:AdaBoostBeta=0.5:SeparationType=GiniIndex:nCuts=-1" );
