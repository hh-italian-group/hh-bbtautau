...
NameElement OptimalBandwidth(const VarData& sample)
{
    ...
    for (const auto& var : sample){
        bandwidth_future[Name_ND{var.first}] = run::async(stat_estimators::OptimalBandwith<double>,
                                                          std::cref(var.second), 0.01);
    }
...
NameElement Mutual(const VarData& sample_vars, NameElement& bandwidth)
{
    ...
    for(auto var_1 = sample_vars.begin(); var_1 != sample_vars.end(); ++var_1){
        for(auto var_2 = var_1; var_2 != sample_vars.end(); ++var_2) {
            ...
            matrix_future[Name_ND{var_1->first, var_2->first}] =
                run::async(stat_estimators::ScaledMutualInformation<double>, std::cref(var_1->second),
                           std::cref(var_2->second), bandwidth.at(Name_ND{var_1->first}),
                           bandwidth.at(Name_ND{var_2->first}));
        }
    }
...
NameElement Correlation(const VarData& sample_vars)
{
    ...
    for(auto var_1 = sample_vars.begin(); var_1 != sample_vars.end(); ++var_1){
        for(auto var_2 = var_1; var_2 != sample_vars.end(); ++var_2) {
            corr_matrix_future[Name_ND{var_1->first, var_2->first}] =
                run::async(stat_estimators::Correlation<double>, var_1->second, var_2->second, 1.);
        }
    }
...
void CreateMatrixHistos(const SampleIdVarData& samples_mass, const SampleIdNameElement& element,
                        const std::string& type, TDirectory* directory, bool draw_diagonal)
{
                                    ...
                                    const NameElement& bandwidth_signal, const NameElement& bandwidth_bkg)
{
    ...
    for(auto entry_1 = sample_signal.begin(); entry_1 != sample_signal.end(); ++entry_1) {
        ...
        JSDivergenceND_future[Name_ND{entry_1->first}] =
            run::async(stat_estimators::JensenShannonDivergence_ND<double>, x, y, band_x, band_y);
        for(auto entry_2 = std::next(entry_1); entry_2 != sample_signal.end(); ++entry_2) {
            ...
            JSDivergenceND_future[Name_ND{entry_1->first, entry_2->first}] =
                run::async(stat_estimators::JensenShannonDivergence_ND<double>,x, y, band_x, band_y);
            x.erase(x.end() - 1);
            y.erase(y.end() - 1);
...
TimeReporter::TimeReporter() : start(clock::now()), start_tot(clock::now()) {}

void TimeReporter::TimeReport(bool tot)
{
...
ChannelSampleIdSpin::ChannelSampleIdSpin() : channel("all_channel"), sample_id(mass_tot), spin(spin_tot) {}
ChannelSampleIdSpin::ChannelSampleIdSpin(std::string _channel, SampleId _sample_id, int _spin)
    : channel(_channel), sample_id(_sample_id), spin(_spin) {}

bool ChannelSampleIdSpin::operator<(const ChannelSampleIdSpin& x) const
{
...
bool ChannelSampleIdSpin::operator !=(const ChannelSampleIdSpin& x) const {return !(x==*this);}

bool ChannelSampleIdSpin::IsAllChannel() const { return channel == all_channel; }
bool ChannelSampleIdSpin::IsAllSpin() const { return spin == spin_tot; }
                                                            ...
                                                            TDirectory* directory)
{
     ...
     for(int i = 0; i<=nbin; ++i){
         ...
         cuts.push_back(OptimalSignificance{output, significance});
                                                                           ...
                                                                           TDirectory* directory, bool total)
{
    ...
    if (spin == SM_spin) {
        ...
        auto cuts = Calculate_CutSignificance(id_sgn_SM, id_bkg, "SM"+channel+"_"+std::to_string(spin)+"_significance",
                                              outputBDT, directory);
        ...
        cuts = Calculate_CutSignificance(id_sgn_SM_allchannel, id_bkg_allchannel, "SM_allchannel_"
                                         + std::to_string(spin) + "_significance", outputBDT, directory);
        sign[id_sgn_SM_allchannel] = cuts.front();
    }
    else{
        auto mass_sign = CreatePlot<TGraphErrors>("Mass_significance_"+channel+"_"+std::to_string(spin),
                                                  "Mass_significance_"+channel+"_"+std::to_string(spin), "mass",
                                                  "S/(sqrt(B))");
        auto mass_sign_allchannel = CreatePlot<TGraphErrors>("Mass_significance_allchannel_"+std::to_string(spin),
                                                             "Mass_significance_allchannel_"+std::to_string(spin),
                                                             "mass", "S/(sqrt(B))");
        int index = 0;
        for(const auto& mass : mass_range){
            ...
            auto cuts = Calculate_CutSignificance(id_sgn_mass, id_bkg_mass, std::to_string(mass) + channel + "_"
                                                  + std::to_string(spin) + "_significance", outputBDT, directory);
            ...
            cuts = Calculate_CutSignificance(id_sgn_mass_allchannel, id_bkg_mass_allchannel, std::to_string(mass)
                                             + "_allchannel_" + std::to_string(spin)+"_significance", outputBDT,
                                             directory);
            sign[id_sgn_mass] = cuts.front();
        ...
        if (total){
            ...
            cuts = Calculate_CutSignificance(id_masstot_allchannel, id_bkg_allchannel, "Significance", outputBDT,
                                             directory);
